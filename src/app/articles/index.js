...
const Controller = PropertyController('articles');
...
export async function createRecord(req, res) {
    ...
    try {
        ...
        if (images && images.coverImage) {
            ...
            resize_save({ file: images.coverImage, fileName: fileName, width: null, height: null }, 'uploads/blog');
        ...
        if (req.files && req.files['otherImages']) {
            ...
            if (typeof (otherImages.name) !== 'undefined') {
                ...
                resize_save({ file: images.otherImages, fileName: fileName, width: null, height:null }, 'uploads/blog');
            } else {
                for (const file of otherImages) {
                    const fileName = recordData.url + '-' + randomString();
                    const filepath = `${fileName}.webp`
                    file_paths.push(filepath);
                    resize_save({ file: file, fileName: fileName, width: null, height:null }, 'uploads/blog');
                };
            }
            // update database
